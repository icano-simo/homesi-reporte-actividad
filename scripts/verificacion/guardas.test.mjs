...
console.log('\n=== exigirAusente: el comentario no cuenta ===');
{
  ...
  /*
   * ⚠ EL SEPTIMO CASO, Y NO ES UN ARCHIVO — etapa OL51. Comprobe si una funcion
   * ya aplicada todavia levantaba `errcode = '40001'` con un regex sobre
   * `pg_proc.prosrc`, dio `true`, y casi reporte que no se habia reaplicado. La
   * version viva traia la correccion Y el comentario que la explica, asi que el
   * patron matcheo el comentario que dice por que el codigo NO esta.
   *
   * `prosrc` es el cuerpo de la funcion con sus comentarios adentro: leerlo es
   * leer un archivo, y vale la misma regla. Se prueban las DOS ramas, porque
   * «no encontro nada» y «no miro» se escriben igual.
   */
  const cuerpoCorregido =
    'begin\n' +
    "  -- SIN `using errcode = '40001'`, a proposito: es serialization_failure\n" +
    '  -- y PostgREST lo reintenta solo hasta agotar el timeout.\n' +
    "  raise exception 'this budget changed while you were editing';\n" +
    'end;';
  const cuerpoViejo = "begin\n  raise exception 'changed' using errcode = '40001';\nend;";
  ck(exigirAusente(cuerpoCorregido, ["using errcode = '40001'"], { lenguaje: 'sql' }) === 1,
     'el cuerpo de una funcion de `pg_proc` guarda SUS comentarios, y tampoco cuentan');
  ck(atrapa(() => exigirAusente(cuerpoViejo, ["using errcode = '40001'"], { lenguaje: 'sql' })) !== null,
     'y con el codigo de verdad puesto, lo atrapa');
...
/* ────────── 7. exigirDefinidos ────────── */
{
  console.log('\n=== exigirDefinidos: lo que se pide tiene que existir ===');

  /* EL CASO DE `bp-hint`: siete usos en el JSX, cero reglas en las hojas. */
  const hojas = '.rv-panel { color: red; }\n.rv-intake { margin: 0; }\n';
  ck(chequearDefinidos(['rv-panel', 'rv-intake'], hojas, { prefijo: '.' }).length === 0,
     'las clases que SI estan definidas no se reportan');
  const faltan = chequearDefinidos(['rv-panel', 'bp-hint', 'rv-intake__body'], hojas,
     { prefijo: '.' });
  ck(faltan.length === 2 && faltan[0] === 'bp-hint' && faltan[1] === 'rv-intake__body',
     'y atrapa las dos que no, ordenadas: ' + JSON.stringify(faltan));

  /*
   * EL CASO DEL PASO 4, que es el que la hizo falta: el guion pide dos
   * identificadores y la pantalla no lleva ninguno. Sin prefijo, porque un
   * `data-review-click` se busca tal cual.
   */
  const pantallaSinMarcas = '<button className="bp-forensic__item">4 loans</button>';
  const pantallaConMarcas = '<button data-review-click="total_pipeline">4</button>' +
    '<button data-review-click="healthy_loans">1</button>';
  ck(chequearDefinidos(['total_pipeline', 'healthy_loans'], pantallaSinMarcas).length === 2,
     'un paso que pide dos clics sobre una pantalla sin marcas: los dos faltan');
  ck(chequearDefinidos(['total_pipeline', 'healthy_loans'], pantallaConMarcas).length === 0,
     'y con las marcas puestas, ninguno');
  /* La mitad exacta: uno marcado y el otro no. Es el caso que hace que el
     mensaje pueda decir CUAL falta. */
  ck(JSON.stringify(chequearDefinidos(['total_pipeline', 'forecast_total'], pantallaConMarcas))
       === '["forecast_total"]',
     'con uno marcado y el otro no, nombra el que falta');

  /* La version que tira, y su mensaje. */
  ck(exigirDefinidos(['rv-panel'], hojas, { prefijo: '.' }) === 1,
     'exigirDefinidos devuelve cuantos comprobo, para poder contarlo en el arnes');
  const msg = atrapa(() => exigirDefinidos(['bp-hint'], hojas,
     { prefijo: '.', que: 'clases', dondeDice: 'las hojas' }));
  ck(msg !== null, 'y falla cuando falta alguno');
  ck(/\.bp-hint/.test(msg) && /las hojas/.test(msg),
     'con el nombre CON su prefijo y donde se busco: ' + JSON.stringify(msg && msg.slice(0, 90)));
  ck(/No falla nada hoy/.test(msg),
     'y explica por que hay que preguntarlo: no falla nada hoy');

  /* El caso degenerado, igual que en `exigirAusente`. */
  ck(atrapa(() => exigirDefinidos([], hojas)) !== null,
     'una lista de pedidos VACIA se rechaza: pasaria siempre');
  ck(atrapa(() => chequearDefinidos('no-es-un-array', hojas)) !== null,
     'y algo que no es un array tampoco pasa en silencio');
}
