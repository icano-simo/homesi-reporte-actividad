 ...
 * ============================================================================
 *
 * Etapa AUTH2. Port de `app/change-password/page.tsx` de homesi-pl, con el
 * estilo traducido a CSS (ver app/styles/auth.css).
 *
 * Llega acá quien tiene sesión válida, acceso a esta app, y todavía
 * `must_change_password: true` en su `app_metadata` -- típicamente porque
 * entró con una contraseña temporal que le asignó un administrador.
 *
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(body.error ?? 'Password changed, but the account could not be unlocked. Try again.');
        return;
      }

      /*
       * El gate lee `app_metadata` del TOKEN, no de la base. Sin refrescar la
       * sesión, el token todavía dice must_change_password: true y el usuario
    ...
    <main className="auth-screen">
      <div className="auth-panel">
        ...
        <div className="auth-card">
          ...
          <p className="auth-subtitle">Your account uses a temporary password. Set your own to continue.</p>
